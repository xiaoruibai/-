# -*- coding: utf-8 -*-
"""Shared ACIC/JOBS two-layer OTL runner for the 50-repeat DR-transfer setup."""
...
DATASET_PREFIX = "jobs"
DEFAULT_TARGET_DATA_DIR = "jobs_ethos_streams_repeats50_target"
DEFAULT_GOOD_SOURCE_DIR = "jobs_ethos_streams_repeats50_source"
DEFAULT_BAD_SOURCE_DIR = "jobs_bad_ethos_streams_repeats50_source"

BASE_SEED = 2026
N_REPEATS = 50
DEFAULT_N_JOBS = 1
DEFAULT_OUTPUT_DIR = "jobs_otl_dr_repeats50_outputs"
...
# Two-expert meta layer (discounted two-layer weighting)
OTL_INITIAL_WEIGHT = 0.5
OTL_LOSS_VARIANT = "two_layer_discounted_half_squared_loss"
OTL_LAMBDA_SCHEDULE = "none"
OTL_META_ETA_SCHEDULE = "sqrt_8ln2_over_t"
OTL_WEIGHT_UPDATE = "discounted_exponential_weights_two_layer"
OTL_WEIGHTING_INTERPRETATION = "engineering_discounted_hedge_for_recoverable_transfer_weights"
OTL_DISCOUNT_GAMMA = 0.98
OTL_WEIGHT_FLOOR = 0.02
...
REPEAT_ENV_FILE_REGEX = re.compile(
    r"^(jobs_(linear|nonlinear)_(switching|linear)_K(\d+))_repeat(\d+)\.csv$"
)
LEGACY_ENV_FILE_REGEX = re.compile(
    r"^(jobs_(linear|nonlinear)_(switching|linear)_K(\d+)\.csv)$"
)

COMPARISON_SPECS = [
    {
        "comparison_label": "ADAPTIVE_DR <- ADAPTIVE_DR (Two Layer)",
        "target_method": "ADAPTIVE_DR",
        "source_method": "ADAPTIVE_DR",
        "baseline_method": "ADAPTIVE_DR",
        "transfer_method": "OTL_ADAPTIVE_DR_FROM_ADAPTIVE_DR_TWO_LAYER",
        "family": "DR",
    },
]
...
METHOD_ORDER = [
    ...
    "ETHOS_IPW",
    "ETHOS_DR",
    "OTL_ADAPTIVE_DR_FROM_ADAPTIVE_DR_TWO_LAYER",
    "OTL_ADAPTIVE_DR_FROM_ADAPTIVE_DR_TWO_LAYER_CLIPPED",
    "OTL_ADAPTIVE_DR_FROM_ADAPTIVE_DR_TWO_LAYER_THETA_ONLY",
    "OTL_ADAPTIVE_DR_FROM_ADAPTIVE_DR_TWO_LAYER_NUISANCE_ONLY",
    "OTL_ADAPTIVE_DR_FROM_ADAPTIVE_DR_TWO_LAYER_BAD_SOURCE",
    "OTL_ADAPTIVE_DR_FROM_ADAPTIVE_DR_TWO_LAYER_BAD_SOURCE_CLIPPED",
]
...
def configure_dataset(
    ...
    global DATASET_PREFIX, DEFAULT_TARGET_DATA_DIR, DEFAULT_GOOD_SOURCE_DIR
    global DEFAULT_BAD_SOURCE_DIR, DEFAULT_OUTPUT_DIR
    global REPEAT_ENV_FILE_REGEX, LEGACY_ENV_FILE_REGEX

    DATASET_PREFIX = dataset_prefix
    DEFAULT_TARGET_DATA_DIR = target_data_dir
    DEFAULT_GOOD_SOURCE_DIR = good_source_dir
    DEFAULT_BAD_SOURCE_DIR = bad_source_dir
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=f"{DATASET_PREFIX} Route-B OTL runner with discounted two-layer weights")
    ...
    parser.add_argument("--two-layer-discount-gamma", type=float, default=OTL_DISCOUNT_GAMMA)
    parser.add_argument("--two-layer-weight-floor", type=float, default=OTL_WEIGHT_FLOOR)
...
def discover_environment_files(data_dir: str) -> List[Dict[str, object]]:
    ...
    if legacy_hits:
        raise FileNotFoundError(
            f"Found legacy {DATASET_PREFIX} environment files without repeat suffix, but no independent-repeat files. "
            f"Expected files like {DATASET_PREFIX}_nonlinear_switching_K5_repeat01.csv. "
            f"Examples found instead: {sorted(legacy_hits)[:5]}"
        )

    raise FileNotFoundError(f"No {DATASET_PREFIX} independent-repeat environment files found in {data_dir}")
...
def resolve_source_path(source_dir: str, target_env_info: Dict[str, object], tag: str = "good") -> str:
    ...
    prefix_candidates = [
        f"{DATASET_PREFIX}_{tag}_",
        f"{DATASET_PREFIX}_good_",
        f"{DATASET_PREFIX}_bad_",
        f"{DATASET_PREFIX}_source_",
    ]
    for prefix in prefix_candidates:
        source_filename = target_filename.replace(f"{DATASET_PREFIX}_", prefix, 1)
    ...
    raise FileNotFoundError(
        f"Cannot find {tag} source file aligned with target repeat file {target_filename} under {source_dir}"
    )
...
def make_transfer_method_name(base_method: str, source_profile: str, loss_mode: str, transfer_line_mode: str) -> str:
    ...
    return str(base_method) if len(parts) == 1 else "_".join(parts).upper()
...
def transform_otl_losses(
    ...
    if loss_mode == "raw":
        return float(loss_old), float(loss_new), 1.0
    if loss_mode == "clipped":
        return float(min(loss_old, OTL_LOSS_CLIP)), float(min(loss_new, OTL_LOSS_CLIP)), float(OTL_LOSS_CLIP)
    if loss_mode == "empirical_scaled":
        loss_scale = float(max(loss_state.get("scale", 1.0), EPS_NUM))
        return (
            float(min(loss_old / loss_scale, OTL_EMPIRICAL_SCALED_LOSS_CLIP)),
            float(min(loss_new / loss_scale, OTL_EMPIRICAL_SCALED_LOSS_CLIP)),
            loss_scale,
        )
...
def update_otl_weights_discounted_two_layer(
    weights: np.ndarray,
    loss_old: float,
    loss_new: float,
    meta_eta: float,
    discount_gamma: float = OTL_DISCOUNT_GAMMA,
    weight_floor: float = OTL_WEIGHT_FLOOR,
) -> np.ndarray:
    """Discount old evidence before a two-expert Hedge update so weights can recover."""
    gamma = float(np.clip(discount_gamma, 0.0, 1.0))
    floor = float(np.clip(weight_floor, 0.0, 0.49))
    base = np.clip(np.asarray(weights, dtype=np.float64), EPS_NUM, 1.0)
    if gamma < 1.0:
        base = np.power(base, gamma)
        base = base / (np.sum(base) + EPS_NUM)

    shifted_old = float(loss_old)
    shifted_new = float(loss_new)
    min_loss = min(shifted_old, shifted_new)
    s_old = math.exp(-float(meta_eta) * (shifted_old - min_loss))
    s_new = math.exp(-float(meta_eta) * (shifted_new - min_loss))
    updated = np.asarray([base[0] * s_old, base[1] * s_new], dtype=np.float64)
    updated = updated / (np.sum(updated) + EPS_NUM)
    if floor > 0.0:
...
# =========================================================
# Route B OTL transfer learners (fixed source + adaptive target, discounted two-layer weights)
# =========================================================
...
def run_exact_otl_transfer_adaptive_core(
    ...
    theta_weights = np.asarray([OTL_INITIAL_WEIGHT, OTL_INITIAL_WEIGHT], dtype=np.float32)
    ...
    if not theta_transfer_enabled:
        theta_weights = np.asarray([0.0, 1.0], dtype=np.float32)
    ...
    acc0_sq = 0.0
    acc1_sq = 0.0
    dr0_weights = np.asarray([OTL_INITIAL_WEIGHT, OTL_INITIAL_WEIGHT], dtype=np.float32)
    dr1_weights = np.asarray([OTL_INITIAL_WEIGHT, OTL_INITIAL_WEIGHT], dtype=np.float32)
    if use_dr and not nuisance_transfer_enabled:
        dr0_weights = np.asarray([0.0, 1.0], dtype=np.float32)
        dr1_weights = np.asarray([0.0, 1.0], dtype=np.float32)
    ...
    dr0_loss_state = init_loss_scale_state()
    dr1_loss_state = init_loss_scale_state()

    for idx, phi in enumerate(Phi):
        pred_old = predict_tau(source_old_model, phi)
        pred_new = predict_tau(theta, phi)
        theta_mix = get_otl_parameter_ensemble(source_old_model, theta, theta_weights) if theta_transfer_enabled else theta
        pred_mix = predict_tau(theta_mix, phi)

        if use_dr:
            alpha0_mix = get_otl_parameter_ensemble(source_alpha0_old, alpha0, dr0_weights) if nuisance_transfer_enabled else alpha0
            alpha1_mix = get_otl_parameter_ensemble(source_alpha1_old, alpha1, dr1_weights) if nuisance_transfer_enabled else alpha1
            m0_source = predict_outcome(source_alpha0_old, phi)
            m1_source = predict_outcome(source_alpha1_old, phi)
            m0_target = predict_outcome(alpha0, phi)
            m1_target = predict_outcome(alpha1, phi)
        ...
        if target_method.endswith("IPW"):
            pseudo_tau = get_ipw_pseudo_outcome(float(w[idx]), float(y[idx]), float(p[idx]))
            pseudo_tau_source = pseudo_tau
            pseudo_tau_target = pseudo_tau
        else:
            pseudo_tau_source = get_dr_pseudo_outcome(
                float(w[idx]), float(y[idx]), float(q[idx]), float(m0_source), float(m1_source)
            )
            pseudo_tau_target = get_dr_pseudo_outcome(
                float(w[idx]), float(y[idx]), float(q[idx]), float(m0_target), float(m1_target)
            )
        ...
        if record_trajectory:
            extra = {
                ...
                "otl_weight_old": float(theta_weights[0]),
                "otl_weight_new": float(theta_weights[1]),
                "theta_weight_source": float(theta_weights[0]),
                "theta_weight_target": float(theta_weights[1]),
                ...
                "two_layer_discount_gamma": float(discount_gamma),
                "two_layer_weight_floor": float(weight_floor),
                ...
                "pseudo_tau_source": float(pseudo_tau_source),
                "pseudo_tau_target": float(pseudo_tau_target),
                "pseudo_tau_mix": float(pseudo_tau),
            }
            if use_dr:
                extra.update({
                    "m0_hat_source": float(m0_source),
                    "m1_hat_source": float(m1_source),
                    "m0_hat_target": float(m0_target),
                    "m1_hat_target": float(m1_target),
                    ...
                    "alpha0_weight_old": float(dr0_weights[0]),
                    "alpha0_weight_new": float(dr0_weights[1]),
                    "alpha1_weight_old": float(dr1_weights[0]),
                    "alpha1_weight_new": float(dr1_weights[1]),
                    "dr0_weight_source": float(dr0_weights[0]),
                    "dr0_weight_target": float(dr0_weights[1]),
                    "dr1_weight_source": float(dr1_weights[0]),
                    "dr1_weight_target": float(dr1_weights[1]),
                })

        tracker.add(pred_mix, float(tau[idx]), int(segment[idx]), int(segment_round[idx]), int(original_t[idx]), extra=extra)

        loss_old, loss_new, loss_gap = get_otl_raw_losses(float(pred_old), float(pred_new), float(pseudo_tau))
        raw_loss_old = float(loss_old)
        raw_loss_new = float(loss_new)
        otl_meta_eta = get_otl_meta_eta(idx + 1, target_method)
        meta_loss_old, meta_loss_new, theta_loss_scale = transform_otl_losses(loss_old, loss_new, str(loss_mode), theta_loss_state)
        if theta_transfer_enabled:
            theta_weights = update_otl_weights_discounted_two_layer(
                theta_weights,
                meta_loss_old,
                meta_loss_new,
                otl_meta_eta,
        ...
        update_loss_scale_state(theta_loss_state, loss_old, loss_new)

        dr_loss_old = np.nan
        dr_loss_new = np.nan
        dr_meta_loss_old = np.nan
        dr_meta_loss_new = np.nan
        dr_loss_scale = np.nan
        nuisance_obs_arm = "none"

        if use_dr:
            if float(w[idx]) == 1.0:
                nuisance_obs_arm = "treated"
                dr_loss_old, dr_loss_new, _ = get_otl_raw_losses(float(m1_source), float(m1_target), float(y[idx]))
                dr_meta_loss_old, dr_meta_loss_new, dr_loss_scale = transform_otl_losses(
                    dr_loss_old, dr_loss_new, str(loss_mode), dr1_loss_state
                )
                if nuisance_transfer_enabled:
                    dr1_weights = update_otl_weights_discounted_two_layer(
                        dr1_weights,
                        dr_meta_loss_old,
                        dr_meta_loss_new,
                        otl_meta_eta,
                ...
                update_loss_scale_state(dr1_loss_state, dr_loss_old, dr_loss_new)
                alpha1, acc1_sq = update_outcome(alpha1, phi, float(y[idx]), acc1_sq, c_eta=1.0)
            else:
                nuisance_obs_arm = "control"
                dr_loss_old, dr_loss_new, _ = get_otl_raw_losses(float(m0_source), float(m0_target), float(y[idx]))
                dr_meta_loss_old, dr_meta_loss_new, dr_loss_scale = transform_otl_losses(
                    dr_loss_old, dr_loss_new, str(loss_mode), dr0_loss_state
                )
                if nuisance_transfer_enabled:
                    dr0_weights = update_otl_weights_discounted_two_layer(
                        dr0_weights,
                        dr_meta_loss_old,
                        dr_meta_loss_new,
                        otl_meta_eta,
                ...
                update_loss_scale_state(dr0_loss_state, dr_loss_old, dr_loss_new)
        ...
        if record_trajectory:
            tracker.rows[-1].update({
                ...
                "otl_theory_loss_scale": float(get_shared_hedge_loss_upper_bound(target_method)),
                "otl_loss_scale": float(theta_loss_scale),
                "otl_err_old_sq": float(2.0 * raw_loss_old),
                "otl_err_new_sq": float(2.0 * raw_loss_new),
                "otl_weight_old_next": float(theta_weights[0]),
                "otl_weight_new_next": float(theta_weights[1]),
                "theta_loss_source": float(loss_old),
                "theta_loss_target": float(loss_new),
                "theta_meta_loss_source": float(meta_loss_old),
                "theta_meta_loss_target": float(meta_loss_new),
                "theta_weight_source_next": float(theta_weights[0]),
                "theta_weight_target_next": float(theta_weights[1]),
            })
            if use_dr:
                tracker.rows[-1].update({
                    "nuisance_obs_arm": nuisance_obs_arm,
                    "nuisance_loss_old": float(dr_loss_old) if not np.isnan(dr_loss_old) else np.nan,
                    "nuisance_loss_new": float(dr_loss_new) if not np.isnan(dr_loss_new) else np.nan,
                    "nuisance_meta_loss_old": float(dr_meta_loss_old) if not np.isnan(dr_meta_loss_old) else np.nan,
                    "nuisance_meta_loss_new": float(dr_meta_loss_new) if not np.isnan(dr_meta_loss_new) else np.nan,
                    "nuisance_loss_scale": float(dr_loss_scale) if not np.isnan(dr_loss_scale) else np.nan,
                    "dr_observed_arm_loss_source": float(dr_loss_old) if not np.isnan(dr_loss_old) else np.nan,
                    "dr_observed_arm_loss_target": float(dr_loss_new) if not np.isnan(dr_loss_new) else np.nan,
                    "dr_observed_arm_meta_loss_source": float(dr_meta_loss_old) if not np.isnan(dr_meta_loss_old) else np.nan,
                    "dr_observed_arm_meta_loss_target": float(dr_meta_loss_new) if not np.isnan(dr_meta_loss_new) else np.nan,
                    "alpha0_weight_old_next": float(dr0_weights[0]),
                    "alpha0_weight_new_next": float(dr0_weights[1]),
                    "alpha1_weight_old_next": float(dr1_weights[0]),
                    "alpha1_weight_new_next": float(dr1_weights[1]),
                    "dr0_weight_source_next": float(dr0_weights[0]),
                    "dr0_weight_target_next": float(dr0_weights[1]),
                    "dr1_weight_source_next": float(dr1_weights[0]),
                    "dr1_weight_target_next": float(dr1_weights[1]),
                })
    ...
    out = tracker.summary()
    theta_param_mix_final = get_otl_parameter_ensemble(source_old_model, theta, theta_weights)
    out.update({
        ...
        "two_layer_discount_gamma": float(discount_gamma),
        "two_layer_weight_floor": float(weight_floor),
        "otl_weight_old_final": float(theta_weights[0]),
        "otl_weight_new_final": float(theta_weights[1]),
        "theta_weight_source_final": float(theta_weights[0]),
        "theta_weight_target_final": float(theta_weights[1]),
        "transfer_strategy": f"two_layer_discounted_parameter_ensemble_{loss_mode}_loss_gamma{float(discount_gamma):.3f}_floor{float(weight_floor):.3f}_{transfer_line_mode}",
    })
    if use_dr:
        out.update({
            ...
            "alpha0_param_mix_final": get_otl_parameter_ensemble(source_alpha0_old, alpha0, dr0_weights),
            "alpha1_param_mix_final": get_otl_parameter_ensemble(source_alpha1_old, alpha1, dr1_weights),
            "alpha0_weight_old_final": float(dr0_weights[0]),
            "alpha0_weight_new_final": float(dr0_weights[1]),
            "alpha1_weight_old_final": float(dr1_weights[0]),
            "alpha1_weight_new_final": float(dr1_weights[1]),
            "dr0_weight_source_final": float(dr0_weights[0]),
            "dr0_weight_target_final": float(dr0_weights[1]),
            "dr1_weight_source_final": float(dr1_weights[0]),
            "dr1_weight_target_final": float(dr1_weights[1]),
            "nuisance_transfer_strategy": "per_arm_discounted_weights_for_dr_pseudo_outcome_layer",
            "nuisance_c_eta": 1.0,
        })
    if record_trajectory:
        out["trajectory_df"] = tracker.frame()
        ...
        for source_profile in source_profiles:
            for loss_mode in loss_modes:
                cases.append({
                    ...
                    "method": make_transfer_method_name(str(spec["transfer_method"]), source_profile, loss_mode, MAIN_TRANSFER_LINE_MODE),
                    "comparison_label": make_comparison_label(spec, source_profile, loss_mode, MAIN_TRANSFER_LINE_MODE),
                })
        if enable_dr_line_ablations:
            for transfer_line_mode in ["theta_only", "nuisance_only"]:
                cases.append({
                    ...
                    "method": make_transfer_method_name(str(spec["transfer_method"]), MAIN_SOURCE_PROFILE, OTL_MAIN_LOSS_MODE, transfer_line_mode),
                    "comparison_label": make_comparison_label(spec, MAIN_SOURCE_PROFILE, OTL_MAIN_LOSS_MODE, transfer_line_mode),
                })
    return cases


def make_transfer_metric_row(env_info: Dict[str, object], seed: int, case: Dict[str, object], out: Dict[str, object]) -> Dict[str, object]:
    spec = case["spec"]
    return attach_env_meta({
        ...
        "two_layer_discount_gamma": float(out.get("two_layer_discount_gamma", np.nan)),
        "two_layer_weight_floor": float(out.get("two_layer_weight_floor", np.nan)),
        ...
        "theta_weight_source_final": float(out.get("theta_weight_source_final", np.nan)),
        "theta_weight_target_final": float(out.get("theta_weight_target_final", np.nan)),
        ...
        "dr0_weight_source_final": float(out.get("dr0_weight_source_final", np.nan)),
        "dr0_weight_target_final": float(out.get("dr0_weight_target_final", np.nan)),
        "dr1_weight_source_final": float(out.get("dr1_weight_source_final", np.nan)),
        "dr1_weight_target_final": float(out.get("dr1_weight_target_final", np.nan)),
...
def run_stage2_repeat(task: Tuple[Dict[str, object], str, Dict[str, str], int, int, int, bool, List[Dict[str, object]], float, float]) -> Dict[str, object]:
    env_info, target_path, source_paths, seed, hidden_dim, batch_size, force_linear, transfer_cases, discount_gamma, weight_floor = task
    env_name = str(env_info["env_name"])
    repeat = int(env_info.get("repeat")) if env_info.get("repeat") is not None else None
    env_target = None
...
def build_stage2_outputs(stage2_results: List[Dict[str, object]]) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    group_cols = [
        ...
        "transfer_line_mode", "theta_transfer_enabled", "nuisance_transfer_enabled",
        "two_layer_discount_gamma", "two_layer_weight_floor", "transfer_strategy"
    ]
    metric_cols = [
        ...
        "theta_weight_source_final", "theta_weight_target_final",
        "dr0_weight_source_final", "dr0_weight_target_final",
        "dr1_weight_source_final", "dr1_weight_target_final",
    ]
...
def run_final_repeat(task: Tuple[Dict[str, object], str, Dict[str, str], int, int, int, bool, bool, List[Dict[str, object]], float, float]) -> Dict[str, object]:
    env_info, target_path, source_paths, seed, hidden_dim, batch_size, force_linear, record_trajectory, transfer_cases, discount_gamma, weight_floor = task
    env_name = str(env_info["env_name"])
    repeat = int(env_info.get("repeat")) if env_info.get("repeat") is not None else None
...
def aggregate_final_method_results(raw_method_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    group_cols = [
        ...
        "otl_share_lambda_schedule", "otl_loss_variant", "otl_loss_mode",
        "otl_weighting_interpretation", "transfer_line_mode", "theta_transfer_enabled",
        "nuisance_transfer_enabled", "two_layer_discount_gamma", "two_layer_weight_floor",
        "transfer_strategy"
    ]
    metric_cols = [
...
def aggregate_trajectory_df(raw_traj_df: pd.DataFrame) -> pd.DataFrame:
    ...
    metric_cols = [
        ...
        "theta_weight_source", "theta_weight_target", "theta_weight_source_next", "theta_weight_target_next",
        ...
        "pseudo_tau_source", "pseudo_tau_target", "pseudo_tau_mix",
        "theta_loss_source", "theta_loss_target", "theta_meta_loss_source", "theta_meta_loss_target",
        "m0_hat_source", "m1_hat_source", "m0_hat_target", "m1_hat_target", "m0_hat_mix", "m1_hat_mix",
        ...
        "nuisance_loss_scale",
        "dr0_weight_source", "dr0_weight_target", "dr0_weight_source_next", "dr0_weight_target_next",
        "dr1_weight_source", "dr1_weight_target", "dr1_weight_source_next", "dr1_weight_target_next",
        "dr_observed_arm_loss_source", "dr_observed_arm_loss_target",
        "dr_observed_arm_meta_loss_source", "dr_observed_arm_meta_loss_target",
    ]
    ...
    for meta_col in ["otl_loss_variant", "otl_loss_mode", "source_profile", "transfer_line_mode", "two_layer_discount_gamma", "two_layer_weight_floor"]:
        if meta_col not in raw_traj_df.columns:
            continue
        variant_df = raw_traj_df[group_cols + [meta_col]].drop_duplicates(subset=group_cols).copy()
...
def build_env_rank_table(env_method_mean_df: pd.DataFrame) -> pd.DataFrame:
    ...
    cols = [
        ...
        "running_mean_mse_mean", "running_rmse_mean", "n_repeats", "c_eta_mean", "otl_meta_eta_mean", "otl_share_lambda_mean",
        "source_profile", "otl_loss_mode", "transfer_line_mode", "two_layer_discount_gamma", "two_layer_weight_floor",
        ...
        "theta_weight_source_final_mean", "theta_weight_target_final_mean",
        "dr0_weight_source_final_mean", "dr0_weight_target_final_mean",
        "dr1_weight_source_final_mean", "dr1_weight_target_final_mean",
    ]
...
def plot_otl_weight_showcase(traj_df: pd.DataFrame, boundaries_df: pd.DataFrame, showcase_env: str, spec: Dict[str, str], output_path: str) -> None:
    ...
    if plot_df.empty or "theta_weight_source" not in plot_df.columns:
        return
    boundaries = boundaries_df[boundaries_df["env_name"] == showcase_env].copy()
    fig, axes = plt.subplots(3, 1, figsize=(9.4, 8.0), sharex=True, constrained_layout=True)
    panels = [
        ("CATE theta layer", "theta_weight_source", "theta_weight_target"),
        ("DR control nuisance layer", "dr0_weight_source", "dr0_weight_target"),
        ("DR treated nuisance layer", "dr1_weight_source", "dr1_weight_target"),
    ]
    for ax, (title, source_col, target_col) in zip(axes, panels):
        if source_col not in plot_df.columns or target_col not in plot_df.columns:
            continue
        ax.plot(plot_df["step"], plot_df[source_col], linewidth=2.0, label="source")
        ax.plot(plot_df["step"], plot_df[target_col], linewidth=2.0, label="target")
        add_segment_lines(ax, boundaries)
    ...
    fig.suptitle(f"{spec['comparison_label']}\nTwo-layer discounted OTL weights", fontsize=13)
    fig.savefig(output_path, dpi=220, bbox_inches="tight")
    plt.close(fig)
...
def main() -> None:
    ...
    target_data_dir = args.target_data_dir or resolve_data_dir([DEFAULT_TARGET_DATA_DIR])
    good_source_dir = args.good_source_dir or resolve_data_dir([DEFAULT_GOOD_SOURCE_DIR])
    ...
    for env_info in env_repeat_files:
        ...
        base_tasks.append((env_info, str(env_info["full_path"]), source_paths, algo_seed))

    print(f"[INFO] DATASET_PREFIX       = {DATASET_PREFIX}")
    ...
    print(f"[INFO] TWO_LAYER_GAMMA     = {float(args.two_layer_discount_gamma):.6f}")
    print(f"[INFO] TWO_LAYER_FLOOR     = {float(args.two_layer_weight_floor):.6f}")
    print(f"[INFO] OTL_LOSS_MODES      = {[OTL_MAIN_LOSS_MODE] + (DEFAULT_ROBUST_LOSS_MODES if not args.disable_robust_loss_variants else [])}")
    print("[INFO] TRANSFER_FOCUS      = DR-only double-line OTL; IPW is retained as target-only/ETHOS baseline")
    ...
    stage2_tasks = [
        (
            env_info, target_path, source_paths, seed, args.hidden_dim, args.feature_batch_size,
            args.force_linear, transfer_cases, float(args.two_layer_discount_gamma), float(args.two_layer_weight_floor)
        )
        for env_info, target_path, source_paths, seed in base_tasks
    ]
    ...
    for env_info, target_path, source_paths, seed in base_tasks:
        ...
        final_tasks.append((
            ...
            record_trajectory,
            transfer_cases,
            float(args.two_layer_discount_gamma),
            float(args.two_layer_weight_floor),
        ))
