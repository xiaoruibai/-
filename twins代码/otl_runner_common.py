# -*- coding: utf-8 -*-
"""Shared Twins OTL runner aligned to the independent-repeat DR-transfer setup."""
...
DEFAULT_TARGET_DATA_DIR = "twins_ethos_streams_repeats10_target"
DEFAULT_GOOD_SOURCE_DIR = "twins_ethos_streams_repeats10_source"
DEFAULT_BAD_SOURCE_DIR = "twins_bad_ethos_streams_repeats10_source"

BASE_SEED = 2026
N_REPEATS = 50
DEFAULT_N_JOBS = 1
DEFAULT_OUTPUT_DIR = "twins_otl_dr_repeats50_outputs"
...
REPEAT_ENV_FILE_REGEX = re.compile(
    r"^(twins_(linear|nonlinear)_(switching|linear)_K(\d+))_repeat(\d+)\.csv$"
)
LEGACY_ENV_FILE_REGEX = re.compile(
    r"^(twins_(linear|nonlinear)_(switching|linear)_K(\d+)\.csv)$"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=f"{DATASET_PREFIX} Route-B OTL runner aligned to independent-repeat data (no share layer)")
...
def run_exact_otl_transfer_adaptive_core(
    ...
    for idx, phi in enumerate(Phi):
        ...
        if theta_transfer_enabled:
            expert_weights = update_otl_weights_exponential_no_share(expert_weights, meta_loss_old, meta_loss_new, otl_meta_eta)
        ...
        if use_dr:
            if float(w[idx]) == 1.0:
                ...
                nuisance_loss_old, nuisance_loss_new, _ = get_otl_raw_losses(float(pred_old_alpha1), float(pred_new_alpha1), float(y[idx]))
                nuisance_meta_loss_old, nuisance_meta_loss_new, nuisance_loss_scale = transform_otl_losses(
                    nuisance_loss_old, nuisance_loss_new, str(loss_mode), alpha1_loss_state
                )
                if nuisance_transfer_enabled:
                    alpha1_expert_weights = update_otl_weights_exponential_no_share(
                        alpha1_expert_weights, nuisance_meta_loss_old, nuisance_meta_loss_new, otl_meta_eta
                    )
                update_loss_scale_state(alpha1_loss_state, nuisance_loss_old, nuisance_loss_new)
                ...
                nuisance_loss_old, nuisance_loss_new, _ = get_otl_raw_losses(float(pred_old_alpha0), float(pred_new_alpha0), float(y[idx]))
                nuisance_meta_loss_old, nuisance_meta_loss_new, nuisance_loss_scale = transform_otl_losses(
                    nuisance_loss_old, nuisance_loss_new, str(loss_mode), alpha0_loss_state
                )
                if nuisance_transfer_enabled:
                    alpha0_expert_weights = update_otl_weights_exponential_no_share(
                        alpha0_expert_weights, nuisance_meta_loss_old, nuisance_meta_loss_new, otl_meta_eta
                    )
                update_loss_scale_state(alpha0_loss_state, nuisance_loss_old, nuisance_loss_new)
...
def aggregate_final_method_results(raw_method_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    group_cols = [
        ...
        "nuisance_transfer_enabled", "transfer_strategy"
    ]
    metric_cols = [
...
def build_env_rank_table(env_method_mean_df: pd.DataFrame) -> pd.DataFrame:
    ...
    cols = [
        ...
        "source_profile", "otl_loss_mode", "transfer_line_mode",
