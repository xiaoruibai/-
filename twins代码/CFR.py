...
# ============================================================
# 1) 全局配置
#    现在改成与 twins 独立 repeat 数据生成脚本对齐：
...
OUTPUT_DIR = "twins_CFR_baseline_prequential_outputs"
BASE_SEED = 2026
N_REPEATS = 10
...
REPEAT_ENV_FILE_REGEX = re.compile(r"^(twins_(linear|nonlinear)_(switching|linear)_K(\d+))_repeat(\d+)\.csv$")
LEGACY_ENV_FILE_REGEX = re.compile(r"^(twins_(linear|nonlinear)_(switching|linear)_K(\d+)\.csv)$")
...
def discover_environment_files(data_dir):
    ...
    if legacy_hits:
        raise FileNotFoundError(
            "发现旧格式 twins 环境文件，但没有发现独立 repeat 文件。"
            "当前脚本期望文件名形如 twins_linear_switching_K5_repeat01.csv。"
            f"目前找到的旧文件示例: {sorted(legacy_hits)[:5]}"
        )

    raise FileNotFoundError(f"在 {data_dir} 中未找到 twins 独立 repeat 环境文件。")
...
def main():
    ...
    raw_csv = os.path.join(OUTPUT_DIR, "twins_offline_baseline_best_by_method_raw_repeat.csv")
    raw_with_stats_csv = os.path.join(OUTPUT_DIR, "twins_offline_baseline_best_by_method_raw_repeat_with_std.csv")
    mean_csv = os.path.join(OUTPUT_DIR, "twins_offline_baseline_best_by_method_repeat_mean.csv")
    best_csv = os.path.join(OUTPUT_DIR, "twins_offline_baseline_env_best_repeat_mean.csv")
