...
N_REPEATS = 10
DEFAULT_N_JOBS = 1
DEFAULT_OUTPUT_DIR = "twins_bad_otl_no_share_outputs"
...
COMPARISON_SPECS = [
    {
        "comparison_label": "ADAPTIVE_IPW <- ADAPTIVE_IPW (No Share)",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="twins Route-B OTL runner aligned to independent-repeat data (no share layer)")
...
def discover_environment_files(data_dir: str) -> List[Dict[str, object]]:
    ...
    if legacy_hits:
        raise FileNotFoundError(
            "Found legacy twins environment files without repeat suffix, but no independent-repeat files. "
            "Expected files like twins_linear_switching_K5_repeat01.csv. "
            f"Examples found instead: {sorted(legacy_hits)[:5]}"
        )

    raise FileNotFoundError(f"No twins independent-repeat environment files found in {data_dir}")
...
def resolve_source_path(source_dir: str, target_env_info: Dict[str, object], tag: str = "good") -> str:
    ...
    # ==============================================================
    source_filename = target_filename.replace("twins_", "twins_good_")
...
def run_exact_otl_transfer_adaptive_core(
    ...
    for idx, phi in enumerate(Phi):
        ...
        theta_mix = get_otl_parameter_ensemble(source_old_model, theta, expert_weights)
        pred_mix = predict_tau(theta_mix, phi)

        if use_dr:
            alpha0_mix = get_otl_parameter_ensemble(source_alpha0_old, alpha0, alpha0_expert_weights)
            alpha1_mix = get_otl_parameter_ensemble(source_alpha1_old, alpha1, alpha1_expert_weights)
        ...
        if record_trajectory:
            tracker.rows[-1].update({
                ...
                "otl_loss_scale": float(loss_scale),
    ...
    out.update({
        ...
        "transfer_strategy": "exponential_weights_parameter_ensemble_raw_loss_no_share",
    })
    if use_dr:
...
def run_stage2_repeat(task: Tuple[Dict[str, object], str, str, int, int, int, bool]) -> Dict[str, object]:
    ...
    for spec in COMPARISON_SPECS:
        out = run_exact_otl_transfer_adaptive_core(
            ...
            source_old_bundle=source_models[str(spec["source_method"])],
            record_trajectory=False,
            repeat=repeat,
...
def build_stage2_outputs(stage2_results: List[Dict[str, object]]) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    group_cols = [
        ...
        "family", "otl_meta_eta", "otl_share_lambda", "otl_share_lambda_schedule", "otl_loss_variant", "transfer_strategy"
    ]
    metric_cols = [
...
def run_final_repeat(task: Tuple[Dict[str, object], str, str, int, int, int, bool, bool]) -> Dict[str, object]:
    ...
    for spec in COMPARISON_SPECS:
        source_old_bundle = source_old_ipw if spec["source_method"] == "ADAPTIVE_IPW" else source_old_dr
        out = run_exact_otl_transfer_adaptive_core(
            env_name=env_name,
            seed=seed,
            env_target=env_target,
...
def aggregate_final_method_results(raw_method_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    group_cols = [
        ...
        "stage1_inherited_c_eta", "otl_meta_eta", "otl_share_lambda", "otl_share_lambda_schedule", "otl_loss_variant", "transfer_strategy"
    ]
    metric_cols = [
...
def aggregate_trajectory_df(raw_traj_df: pd.DataFrame) -> pd.DataFrame:
    ...
    metric_cols = [
        ...
        "otl_loss_gap", "otl_loss_old_raw", "otl_loss_new_raw", "otl_loss_scale",
        ...
        "nuisance_loss_old", "nuisance_loss_new",
    ]
...
def main() -> None:
    ...
    target_data_dir = args.target_data_dir or resolve_data_dir([
        "twins_ethos_streams_repeats10_target",

    ])
    good_source_dir = args.good_source_dir or resolve_data_dir([
        "twins_bad_ethos_streams_repeats10_source",
    ...
    print("[INFO] OTL_META_LOSS      = raw 0.5*(pred - pseudo_tau)^2 (no loss scaling)")
