...
推荐：
python twins_otl_exact_adaptive_indep_runner_no_share.py --n-repeats 10 --n 1
"""
...
N_REPEATS = 10
DEFAULT_N_JOBS = 1
DEFAULT_OUTPUT_DIR = "twins_otl_no_share_outputs"
...
def update_outcome(theta: np.ndarray, phi: np.ndarray, y: float, acc_sq: float) -> Tuple[np.ndarray, float]:
    ...
    eta = get_scale_free_eta(acc_sq=acc_sq, radius=OUTCOME_NUISANCE_PARAM_NORM_BOUND, c_eta=1.0)
...
def fit_source_old_classifier_adaptive(env_source: Dict[str, object], method: str) -> np.ndarray:
    ...
    return run_adaptive_core("__source__", 0, source_env, method, record_trajectory=False, main_c_eta=1.0/math.sqrt(2.0))["theta_final"].astype(np.float32)
...
def main() -> None:
    ...
    good_source_dir = args.good_source_dir or resolve_data_dir([
        "twins_ethos_streams_repeats10_source",
