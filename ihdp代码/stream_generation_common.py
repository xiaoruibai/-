...
def is_binary_01_series(s: pd.Series) -> bool:
    ...
    if len(non_na) == 0:
        return False
    return set(np.unique(non_na).tolist()).issubset({0.0, 1.0})
...
def validate_known_multiclass_columns(Xraw: pd.DataFrame, categorical_cols: list[str]) -> None:
    ...
    for canon_name, allowed_values in EXPECTED_CATEGORIES.items():
        ...
        obs = set(Xraw[raw_col].dropna().astype(str).str.strip().unique().tolist())
...
def preprocess_covariates(df: pd.DataFrame, dataset_label: str) -> tuple[np.ndarray, list[str]]:
    ...
    df = df[keep].copy()
    Xraw = df[pick_covariate_columns(df)].copy()
...
def generate_one_stream(
    ...
    n = X_all.shape[0]
    online_idx_full = rng.choice(n, size=config.t, replace=config.t > n)
    ...
    for seg, start, end in build_segment_slices(config.t, K):
...
def run_generation(config: StreamGenerationConfig) -> None:
    ...
    child_seqs = np.random.SeedSequence(config.base_seed).spawn(len(configs) * config.repeats)
