...
OUTDIR = "ihdp_ethos_streams_repeats50_target"
...
def preprocess_covariates(df: pd.DataFrame) -> tuple[np.ndarray, list[str]]:
    ...
    print("[INFO] ihdp preprocessing complete:")
...
def mu_nonlinear(X: np.ndarray, beta3: np.ndarray):
    """Original ihdp-style nonlinear response surface."""
    ...
    for seg, start, end in segment_slices:
        ...
        for j in range(L):
            ...
            row: dict[str, float | int | str] = {
                "env_name": f"ihdp_nonlinear_{drift}_K{K}_repeat{repeat_id:02d}",
                "surface": "nonlinear",
                "drift": drift,
                "K": int(K),
...
def main() -> None:
    ...
    for drift, K in configs:
        for repeat_id in range(1, REPEATS + 1):
            ...
            fname = f"ihdp_nonlinear_{drift}_K{K}_repeat{repeat_id:02d}.csv"
    ...
    print("\n[ALL DONE] All nonlinear-only ihdp online-stream CSVs were generated.")
