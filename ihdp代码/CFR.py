...
OUTPUT_DIR = "ihdp_offline_baseline_prequential_outputs"
BASE_SEED = 2026
N_REPEATS = 10
...
def discover_environment_files(data_dir):
    ...
    if legacy_hits:
        raise FileNotFoundError(
            "发现旧格式 ihdp 环境文件，但没有发现独立 repeat 文件。"
            "当前脚本期望文件名形如 ihdp_linear_switching_K5_repeat01.csv。"
            f"目前找到的旧文件示例: {sorted(legacy_hits)[:5]}"
        )

    raise FileNotFoundError(f"在 {data_dir} 中未找到 ihdp 独立 repeat 环境文件。")
...
def main():
    ...
    raw_csv = os.path.join(OUTPUT_DIR, "ihdp_offline_baseline_best_by_method_raw_repeat.csv")
    raw_with_stats_csv = os.path.join(OUTPUT_DIR, "ihdp_offline_baseline_best_by_method_raw_repeat_with_std.csv")
    mean_csv = os.path.join(OUTPUT_DIR, "ihdp_offline_baseline_best_by_method_repeat_mean.csv")
    best_csv = os.path.join(OUTPUT_DIR, "ihdp_offline_baseline_env_best_repeat_mean.csv")
