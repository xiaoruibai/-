...
OUTDIR = "ihdp_bad_ethos_streams_repeats50_source"
...
def pick_covariate_columns(df: pd.DataFrame) -> list[str]:
    """Select ihdp covariate columns by exclusion rules, preserving ihdp structure."""
    exclude_substrings = [
...
def preprocess_covariates(df: pd.DataFrame) -> tuple[np.ndarray, list[str]]:
    """Process raw ihdp covariates into a numeric matrix while preserving ihdp features."""
    ...
    for seg, start, end in segment_slices:
        ...
        for j in range(L):
            ...
            row: dict[str, float | int | str] = {
                "env_name": f"ihdp_good_nonlinear_{drift}_K{K}_repeat{repeat_id:02d}",
                "surface": "nonlinear",
                "drift": drift,
                "K": int(K),
...
def main() -> None:
    ...
    for drift, K in configs:
        for repeat_id in range(1, REPEATS + 1):
            ...
            fname = f"ihdp_good_nonlinear_{drift}_K{K}_repeat{repeat_id:02d}.csv"
    ...
    print("\n[ALL DONE] All ihdp good/source nonlinear-only repeat streams generated successfully.")
