...
# ============================================================
# 1) 全局配置
#    现在改成与 jobs 独立 repeat 数据生成脚本对齐：
#      - 读取 jobs_{surface}_{drift}_K{K}_repeat01.csv ... repeat10.csv
...
TARGET_DATA_DIR = resolve_data_dir([
    "jobs_ethos_streams_repeats10_target",
])

OUTPUT_DIR = "jobs_CFR_baseline_prequential_outputs"
BASE_SEED = 2026
N_REPEATS = 10
...
# 只保留 CFR 方法，窗口网格只取 CFR 需要的那一个
METHODS_TO_RUN = ["CFR"]
METHOD_WINDOW_GRID = {
    "CFR": [100],      # 可在此调整窗口大小，如 [50, 100] 等
}
...
REPEAT_ENV_FILE_REGEX = re.compile(r"^(jobs_(linear|nonlinear)_(switching|linear)_K(\d+))_repeat(\d+)\.csv$")
LEGACY_ENV_FILE_REGEX = re.compile(r"^(jobs_(linear|nonlinear)_(switching|linear)_K(\d+)\.csv)$")
...
def discover_environment_files(data_dir):
    ...
    if legacy_hits:
        raise FileNotFoundError(
            "发现旧格式 jobs 环境文件，但没有发现独立 repeat 文件。"
            "当前脚本期望文件名形如 jobs_linear_switching_K5_repeat01.csv。"
            f"目前找到的旧文件示例: {sorted(legacy_hits)[:5]}"
        )

    raise FileNotFoundError(f"在 {data_dir} 中未找到 jobs 独立 repeat 环境文件。")
...
def fit_cf_cate(X, w, y, config, seed):
    """
    CF (Causal Forest) 作为离线基线方法，使用 forest-based DR learner 近似。
    注意：此处保留该函数供 CFR 内部使用（CFR 训练前需要估计倾向得分和结果模型）。
    """
    mask1 = (w == 1)
    mask0 = (w == 0)
...
def fit_cfr_cate(X, w, y, config, seed):
    """
    CFR (Counterfactual Regression) 使用 Sinkhorn 距离做表征分布对齐。
    该方法继承自 TARNet，通过 method_name="CFR" 调用。
    """
    configure_torch_runtime()
    set_global_seed(seed)
...
def fit_cate_model(method, X, w, y, config, seed):
    if method == "CFR":
        return fit_cfr_cate(X, w, y, config, seed)
...
def build_method_grid(method):
    ...
    if method == "CFR":
        for window in method_windows:
            for alpha in [0.1, 0.3]:           # 表征对齐强度
                for hidden_dim in [32, 64]:     # 隐藏层宽度
                    configs.append({
                        "window": int(window),
                        "lr": 1e-3,
                        "weight_decay": 1e-4,
...
def main():
    ...
    raw_csv = os.path.join(OUTPUT_DIR, "jobs_offline_baseline_best_by_method_raw_repeat.csv")
    raw_with_stats_csv = os.path.join(OUTPUT_DIR, "jobs_offline_baseline_best_by_method_raw_repeat_with_std.csv")
    mean_csv = os.path.join(OUTPUT_DIR, "jobs_offline_baseline_best_by_method_repeat_mean.csv")
    best_csv = os.path.join(OUTPUT_DIR, "jobs_offline_baseline_env_best_repeat_mean.csv")
