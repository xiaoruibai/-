...
N_REPEATS = 10
DEFAULT_N_JOBS = 1
DEFAULT_OUTPUT_DIR = "acic16_l1_tldr_fixed_window100_preq_outputs"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="acic16 offline TL with fixed-window prequential evaluation using oracle p/q")
...
def discover_environment_files(data_dir: str) -> List[Dict[str, object]]:
    ...
    if legacy_hits:
        raise FileNotFoundError(
            "Found legacy acic16 environment files without repeat suffix, but no independent-repeat files. "
            "Expected files like acic16_linear_switching_K5_repeat01.csv. "
            f"Examples found instead: {sorted(legacy_hits)[:5]}"
        )

    raise FileNotFoundError(f"No acic16 independent-repeat environment files found in {data_dir}")
...
def resolve_source_path(source_dir: str, target_env_info: Dict[str, object], tag: str = "good") -> str:
    ...
    source_filename = target_filename.replace("acic16_", "acic16_good_") if tag == "good" else target_filename.replace("acic16_", "acic16_good_")
...
def main() -> None:
    ...
    target_data_dir = args.target_data_dir or resolve_data_dir(["acic16_ethos_streams_repeats10_target"])
    good_source_dir = args.good_source_dir or resolve_data_dir(["acic16_ethos_streams_repeats10_source"])
