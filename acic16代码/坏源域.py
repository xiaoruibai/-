...
def main() -> None:
    run_generation(
        StreamGenerationConfig(
            ...
            outdir="acic16_bad_ethos_streams_repeats50_source",
            t=3000,
            repeats=50,
            eps=0.05,
            noise_var=0.1,
            base_seed=520,
            prop_subsample=3000,
            env_prefix="acic16_bad",
            beta_sampler=sample_beta,
            response_surface=mu_nonlinear,
            propensity_gamma_scale=1.5,
            done_message="All acic16 bad/source nonlinear-only repeat streams generated successfully.",
        )
    )
